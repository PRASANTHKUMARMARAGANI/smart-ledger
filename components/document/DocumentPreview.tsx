    ...
    <div className="bg-slate-900 text-slate-100 rounded-2xl p-6 sm:p-8 shadow-lg border border-slate-800 space-y-6 flex flex-col justify-between min-h-[500px]">
      ...
      <div className="flex items-center justify-between border-b border-slate-800 pb-4">
        ...
        <div className="flex items-center gap-1.5 text-[11px] text-slate-400 bg-slate-800 px-2.5 py-1 rounded-lg">
          ...
          <span>Extracted Document Sheet</span>
        </div>
      </div>

      {/* Visual Rendered Document Sheet displaying actual extracted values */}
      <div className="bg-white text-slate-900 rounded-xl p-6 sm:p-8 shadow-2xl space-y-6 text-xs font-sans border border-slate-200">
        ...
        <div className="flex justify-between items-start border-b border-slate-200 pb-6 gap-4">
          <div className="space-y-1 max-w-[60%]">
            <h2 className="text-xl font-extrabold text-sky-950 tracking-tight">{doc.vendor || 'Unspecified Vendor'}</h2>
            {doc.vendorAddress && <p className="text-slate-600 text-[10px] leading-relaxed">{doc.vendorAddress}</p>}
            {doc.vendorGstin && <p className="text-slate-500 text-[10px] font-mono">GSTIN: {doc.vendorGstin}</p>}
            {(doc.vendorPhone || doc.vendorEmail) && (
              <p className="text-slate-500 text-[10px]">
                {doc.vendorPhone ? `Phone: ${doc.vendorPhone}` : ''}
                {doc.vendorPhone && doc.vendorEmail ? ' | ' : ''}
                {doc.vendorEmail ? `Email: ${doc.vendorEmail}` : ''}
              </p>
            )}
          </div>
        ...
        {(doc.billToCustomer || doc.billToAddress || doc.billToGstin) && (
          <div className="grid grid-cols-2 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-200/80 text-[11px]">
            <div>
              ...
              {doc.billToCustomer && <span className="font-extrabold text-slate-900 block text-xs">{doc.billToCustomer}</span>}
              {doc.billToAddress && <p className="text-slate-600 leading-tight mt-0.5">{doc.billToAddress}</p>}
              {doc.billToGstin && <p className="text-slate-500 font-mono text-[10px] mt-1">GSTIN: {doc.billToGstin}</p>}
            </div>

            <div>
              <span className="font-bold text-slate-900 block uppercase tracking-wider text-[10px] mb-1">Ship To</span>
              <span className="font-extrabold text-slate-900 block text-xs">{doc.billToCustomer || 'Same as Billing'}</span>
              {doc.billToAddress && <p className="text-slate-600 leading-tight mt-0.5">{doc.billToAddress}</p>}
            </div>
          </div>
        )}
          ...
          {doc.items && doc.items.length > 0 ? (
            doc.items.map((item, idx) => (
              <div key={idx} className="grid grid-cols-12 text-[11px] py-2 border-b border-slate-100 text-slate-800 items-center">
                ...
                <div className="col-span-2 text-center text-slate-500 font-mono">{item.hsnSac || '-'}</div>
            ...
            <div className="py-4 text-center text-xs text-slate-400 italic">No line items extracted from uploaded document.</div>
          )}
        </div>
        ...
        <div className="pt-4 border-t border-slate-200 space-y-3 text-[11px]">
          {doc.amountInWords && (
            <div>
              ...
              <p className="text-slate-700 font-medium italic">{doc.amountInWords}</p>
            </div>
          )}
          ...
          {doc.notes && (
            <div>
              ...
              <p className="text-slate-600 whitespace-pre-line leading-relaxed">{doc.notes}</p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-4 border-t border-slate-100">
            <div className="flex items-center gap-1.5 text-emerald-700 text-[10px] font-bold">
              ...
              <span>SmartLedger AI Vision Document Processing Passed</span>
            </div>
            {doc.signatory && (
              <div className="text-right">
                <p className="font-bold text-slate-900 text-xs">For {doc.vendor}</p>
                <p className="font-bold text-slate-700 text-xs italic mt-1">{doc.signatory}</p>
